                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                        ...
                        <tbody>
                            @foreach ($sliders as $slider)
                                <tr>
                                    <td>{{ $slider->{'name_' . app()->getLocale()} }}</td>
                                    <td>{!! $slider->{'desc_' . app()->getLocale()} !!}</td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{ route('sliders.edit', ['lang' => app()->getLocale(), 'slider' => $slider->id]) }}"
                                            class="btn btn-warning">{{ __('pages.edit') }}</a>
                                        <form id="delete-form-{{ $slider->id }}"
                                            action="{{ route('sliders.destroy', ['lang' => app()->getLocale(), 'slider' => $slider->id]) }}"
                                            method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <a href="javascript:void(0);" onclick="confirmDelete({{ $slider->id }})"
