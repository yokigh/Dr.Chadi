...
@section('title', __('pages.edit_category'))
...
@section('content')
    ...
    <form action="{{ route('category.update', ['lang' => app()->getLocale(), 'category' => $category->id]) }}" method="POST" enctype="multipart/form-data" class="row">
        @csrf
        @method('PUT')
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">{{ __('pages.edit_category') }}</h4>
                    ...
                    @foreach ($languages as $lang)
                        ...
                        <div class="form-group mb-3 row">
                            ...
                            <div class="col-md-10">
                                <textarea class="form-control" id="desc_{{ $lang }}" name="desc_{{ $lang }}">{{ old('desc_' . $lang, $category->{'desc_' . $lang}) }}</textarea>
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div id="single-drop-area" class="drop-area">
                        ...
                        <img id="single-image-preview" 
                             ...
                             alt="Image Preview"
                             style="{{ $category->image ? '' : 'display: none;' }} margin-top: 10px; max-width: 100%;">
                    </div>
                    ...
                    <div id="multi-drop-area" class="drop-area mt-4">
                        ...
                        <div id="image-preview-container" class="row mt-3">
                            @if ($category->images)
                                @foreach (json_decode($category->images, true) as $img)
