...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">{{ __('pages.editcontact') }}</h4>
                    <p class="card-title-desc">{{ __('pages.update_contact_info') }}</p>
                    ...
                    <form action="{{ route('contacts.update', ['lang' => app()->getLocale(), 'contact' => $contact->id]) }}"
                        ...
                        <button type="submit" class="btn btn-primary mt-3">{{ __('pages.update') }}</button>
